...
export function TelegramSettings({ initialConfig }: TelegramSettingsProps) {
	...
	async function handleLoadPairings() {
		...
		try {
			...
			if (!response.ok || !payload?.pairings) {
				setPairingError(payload?.error ?? "Unable to load pairings");
				return;
			}
	...
	async function handleApprovePairing() {
		...
		if (!code) {
			setPairingError("Pairing code is required.");
			return;
		}
		...
		try {
			...
			if (!response.ok || !payload?.approved) {
				setPairingError(payload?.error ?? "Unable to approve pairing");
				return;
			}

			const displayName = payload.approved.userName || payload.approved.userId;
			setPairingCode("");
			setPairingSuccess(`Approved ${displayName || "Telegram user"}.`);
						...
						<>
							...
							<section className="island-shell rounded-[2rem] p-6 sm:p-8">
								<div className="mb-6 flex flex-col gap-3">
									...
									<p className="m-0 max-w-2xl text-sm text-[var(--sea-ink-soft)] sm:text-base">
										When Hermes sends a pairing code in Telegram, paste it here
										to approve that user.
									</p>
								</div>
									...
									<>
										<div className="space-y-2">
											...
											<div className="flex gap-2">
												...
												>
													...
													<span>
														{isApprovingPairing ? "Approving..." : "Approve"}
													</span>
												</Button>
											</div>
										</div>
										...
										<div className="mt-4 flex flex-wrap gap-3">
											...
											>
												...
												<span>
													{isLoadingPairings ? "Refreshing..." : "Refresh"}
												</span>
											</Button>
										</div>
