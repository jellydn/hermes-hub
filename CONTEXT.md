...
**Legacy Plaintext API Server Key**: A pre-AES-256-GCM credential stored verbatim in `telegram_configs.api_server_key` (auto-generated during the Telegram bot deploy via `crypto.randomBytes(32)` and persisted with `encryptSecret`). This column is the SOLE caller of `decryptApiServerKey` — `ai_providers.encrypted_api_key` uses `decryptSecret` directly, not the legacy-plaintext-tolerant wrapper. Before plan 005, `decryptApiServerKey` silently returned any payload without a ``.`` separator as-is — meaning a corrupted byte, a misclassified placeholder, or attacker-controlled plaintext was treated as a valid bearer credential and shipped into a remote `Authorization: Bearer …` header. Now `decryptApiServerKey` throws on plaintext inputs and logs a structured pino warn. Every direct caller (`server/telegram.ts:testTelegramBot`, `server/deploy.ts:deployProviderToHermes`, `server/server-compose.ts`) catches the throw and surfaces a 502 / composed error carrying `error.message`. The exact operator-facing message is `"API server key is in legacy plaintext format and cannot be decrypted; the operator must redeploy the Telegram bot to regenerate this key."` Remediation: hit `POST /api/telegram/deploy` (`server/telegram.ts:deployTelegramToServer`) — the deploy flow mints a fresh `apiServerKey` via `crypto.randomBytes(32).toString("hex")`, encrypts it via `encryptSecret`, and writes it back to `telegram_configs.api_server_key`. On next read the column holds a properly-encrypted value, unblocking the call/deploy path. This entry is the canonical operator runbook for the new 502.
