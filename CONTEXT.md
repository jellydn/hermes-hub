...
**Legacy Plaintext API Server Key**: A pre-AES-256-GCM credential stored verbatim in `telegram_configs.api_server_key` (or `ai_providers.encrypted_api_key` if a column migration landed late). Before plan 005, `decryptApiServerKey` silently returned any payload without a ``.`` separator as-is — meaning a corrupted byte, a misclassified placeholder, or attacker-controlled plaintext was treated as a valid bearer credential and shipped into a remote `Authorization: Bearer …` header. Now `decryptApiServerKey` throws on plaintext inputs and logs a structured pino warn. Every direct caller (`server/telegram.ts:testTelegramBot`, `server/deploy.ts:deployProviderToHermes`, `server/server-compose.ts`) catches the throw and surfaces a 502 carrying `error.message`. The exact operator-facing message is `"API server key is in legacy plaintext format and cannot be decrypted; the operator must re-save the provider via /api/providers."` Remediation: hit `POST /api/providers` (`server/providers.ts`) with the provider's `apiKey` — the POST route re-runs `encryptSecret` on the new payload and overwrites the column, which on next read uses the encrypted path correctly and unblocks the call/deploy. This entry is the canonical operator runbook for the new 502.
